...
class Mutabilities(object):
    ...
    def __init__(self, element: str, chromosome: str, start: int, end: int, config: dict):
    ...
    def get_mutability_by_position(self, position: int) -> dict:
        """
    ...
    def _load_mutabilities(self):
        ...
        try:
            with mutabilities_reader as reader:
                try:
                    for row in reader.get(self.chromosome, self.start, self.end, self.element):
                        mutability, ref, alt, pos = row
                        # ref_triplet = get_ref_triplet(self.chromosome, pos - 1)
            ...
            logger.warning("Reader error: %s. Regions being analysed %s", e.message, self.element)